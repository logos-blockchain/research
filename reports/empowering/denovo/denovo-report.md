...
**Block space does not stay comfortable, and this makes §8.3 non-negotiable.** On the specification's own mining-cost basis — three Poseidon2 permutations per attempt, the precomputation the Mantle text itself describes — a ×100 spike epoch offers **1,769–1,837 claims a block against 1,024 of space and pays exactly the cap, in every independent draw**. The whole epoch is pegged: ordinary transactions are crowded out completely, not partially. This paragraph has moved twice with the measurement basis, and both superseded readings stay named: 190 average with a 240 peak was the one-core seating, and 730–759 average with 853–863 peaks (83–84% of the cap) was our measured naive miner — a basis the 2026-09 Mantle text now contradicts; a shared-draw path was also once misquoted as "fills the cap outright, 958 average". The engine clips claims at `max_block_txs` alone, with no reservation for the ordinary traffic the fee flow assumes, and the same cap now rations onboarding itself: a front-loaded field converts **5,069 of its 28,600 arrivals** and a back-loaded one 12–23% across draws, where the naive basis converted all of the first and 76–100% of the second. **MODEL §8.3 needs a reservation rule — a cap on the share of a block that claims may take — and the redesign must not be adopted without one.** It is gated (`the block-space cap DOES bind in a x100 spike epoch`, plus the flipped conversion gates that record what the cap now costs) so it cannot quietly re-close.

**The acceptance window prices this crowding — and the spike no longer escapes it.** Every claim is anchored to a recent block and dies if not included within `W = 10` blocks (mantle's grinding defence; since the 2026-09 revision a claim may also straddle one epoch boundary, which is exactly the inclusion semantics this model's persistent queue assumed). Modelled exactly — a FIFO inclusion queue at the *unclipped* demand, which the engine's cap-clip otherwise hides — the regime that retires stays free: worst epoch inflation 1.000×, expiry 0.00%. Everything else now pays. The ×100 spike epoch loses **44.2% of its solutions** to the window and burns ×1.79 energy per paid claim: 1,837 offered a block clears 1,024 of space only by dying in the queue. The late persistent-regime bootstrap is worse — epoch 100 already expires 44.8% of solutions at ×1.81, and epoch 194: 3,483 offered a block, **70.6% of solutions expire**, energy per paid claim ×3.40. Closed through the retirement decision (`window.congested_price_curve`), the tax now moves a threshold: at $0.10 the taxed field retires **19 epochs earlier** (88 against 107), while $1, $0.05 and $0.01 stay unmoved with bond totals shifting about 1%. The post-phase saturation tail keeps its small standing fee (§6). On the naive-miner basis every number in this paragraph was benign — 0% spike expiry, 28.8% and ×1.41 at epoch 194, no threshold moved anywhere — and that superseded reading is kept in the gates' notes.
...
| --- | --- | --- |
| uniform | 24,674 | 7,635 |
| ×10 spike | 25,971 | 7,645 |
| ×100 spike | 33,882 | 7,010 |
| front-loaded | 5,069 | 2,807 |
| back-loaded | 4,935 | 2,760 |
...
One small standing cost the throttle's own success creates, now priced: a solution found after the saturation point waits for the next epoch's budget, and the acceptance window (`W = 10` blocks) forgives only the final ten blocks of that wait — everything found between saturation and that grace strip expires and must be re-mined. Measured: **0.137% of a settled post-phase epoch's solutions**, per epoch (`window.post_tail_loss`, gated). Real, small, and worth knowing it is the R7b design choice's only hidden fee.

R8 holds by construction in both regimes — the reward is floored at the anchor during bootstrap and equals it afterwards — but the 2026-09 upstream change broke the arithmetic underneath it: the claim now carries a ZkSignature and 590 execution gas, so **its own fee (11,298 lepta) exceeds the 11,158-lepta anchor by 140 lepta**, where it used to consume 59.7% of it. A post-phase claim at the anchor mines at a small loss; R1's self-funding margin is gone exactly where the design leans on it. **This is an open design decision** — the natural re-strikes are an anchor of three average transfers (16,737 lepta) or of the claim's fee plus one transfer (16,877) — and it is gated (`the anchor NO LONGER clears the claim's own fee`) so the report cannot outlive the defect silently. SUMMARY carries the same warning.
