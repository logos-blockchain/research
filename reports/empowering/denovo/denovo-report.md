...
How long those 85 claims take is the on-ramp's defining property, because the redesign prices time differently from the current mechanism. The epoch's sub-pool funds about **256 bonds' worth of value** (256,410 LGO over the 1,000-LGO bond) *whatever the field's hashrate* — the reward indexes down as claimants multiply, so more competition means smaller rewards more often, not a longer wait. A miner arriving in a flow at or below that rate earns its bond **within the epoch it arrives** (measured: uniform cohorts bond in their arrival epoch, and their neighbours before a spike do the same), and only a queue slows it — the ×100 crunch of §4 stretches the median to 64 epochs, because 13,000 bonds take about 51 epochs of budget to fund. The current design prices the same climb by field *share* against a decaying pool: about a month of mining at a thousand identical competitors, eleven months at ten thousand, and never past fifty thousand (the strategy report's §5 tabulates it). Queue-priced against a flat schedule, against share-priced against a decaying one — that is the whole difference in one sentence.
