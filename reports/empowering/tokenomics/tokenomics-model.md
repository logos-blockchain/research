...
## 0.6 Addendum — the mining precomputation withdrawn; the Blend candidate re-costed (2026-09-09, latest)

Review round two of logos-lips PR #400 withdrew the sponge-prefix shortcut on both puzzles. The Blend ticket is now `zkhash(pow_nonce, pol_epoch_nonce)` — two inputs, the searched nonce first, no domain separation tag — and the reward ticket `zkhash(public_key, block_hash, epoch_nonce)` with the searched key first. Nothing constant is absorbed before the searched input, so no sponge state carries from one candidate to the next; the optimiser's edge is now 1.00×. Consequences for this report:

- **The Blend candidate is three Poseidon2 permutations, measured on the target.** Three pinned runs on the Pi 5 on 2026-09-09 with the current form (spread 0.0 %, rustc 1.94.0, logos-blockchain `6efd15a`) put a candidate at **72.8 μs**, against 94.2 μs for the four-permutation v0.5.6 form; the August runs' two-input line, 71.3 μs, was within 2 %. At the specified `field_modulus / 2**19` a solution is ~38 s and ~2,270 messages/day per core, and four threads sustain 4.00× one core, measured rather than assumed. The exponent stays at 19, and the specification's calibration sentence is restated at the measured figure.
- **The reward candidate is unchanged in cost**: the key derivation plus a three-input ticket, seven permutations, 168.6 μs measured, or 145.6 μs with the constant `KDF` tag absorbed ahead of time, also measured. The genesis reward target keeps `field_modulus / 2**26`, ~3.1 hours per solution on one core.
- **The gates follow the specification split.** The reward pool, the genesis seed and both difficulty controllers moved from Mantle into `proof-of-work.md`; `make check` reads them there when the document exists and from Mantle otherwise, and the withdrawn `BLEND_POW_V1` tag is no longer a required phrase.

## 0.5 Addendum — the substrate moves to pooling/distributing/releasing (lips PR 375; 2026-08-24)
...
- **It improves bootstrap security**: the same endowment distributes at half the rate, so the peak adversarial stake share falls from 0.51 % to **0.42 %** (`adversary_hashrate = 0.33`, `initial_stake = 30 %`), and the opening over-payment halves (`opening_reward` = 102,444× the fee, was 204,889×; both figures are on the 2026-09 signed-claim fee).
- **It spends no live margin**: `reward_over_fee` (5.02 then, 2.96 since the 2026-09 claim signature), the builder edge, the break-even load and the claim-share ceiling are all `distribution_rate`-free; `pow_share` stays at 10 %, keeping the subordination headroom and no reliance on the unset leader share; `target_claims_per_block` stays at 10, keeping every §4.4.1 argument.
...
Consequences worth naming: the pool's half-life is 138 epochs ≈ 2.8 years (was 69 ≈ 1.4); the reserve `steady_pool` is 1,446 LGO and the floor `pool_floor` 488; the descent to the fee-funded regime takes ~43 years at the resting price (0.3 at the deflation threshold); the endowment sustains claiming for ~47 years with no traffic at all; and §4.11's window becomes (5.12, 11.76], which **contains the specified `target_claims_per_block` = 10** — the (11, 11 %) observation is discharged rather than adopted. Figures in §§0.0–0.3 are at the pre-change 1/100 and are kept as the historical record.
...
- **The Blend candidate is one 3-input hash**: measured 14.9 μs naive / 8.2 μs with the constant `(dst, epoch_nonce)` prefix precomputed — an algorithmic edge of **1.81×** (was 1.40×), since the constant prefix is now half the naive work. Re-deriving `BLEND_DIFFICULTY_BASE` at the cheaper candidate **leaves `field_modulus / 2**22` in place**: 62 s of one M4 core per message, ~1,400 messages a day, still 4–8× slower on the Pi 5 pending its measurement.
...
- **The Pi 5 measurement is in, and the threshold is calibrated on it.** Six runs on the target board (spreads ≤ 0.1 %, no throttling): a blend candidate costs 94.2 μs — 6.3× the desktop figure, inside the 4–8× band the model carried as an estimate. The reference basis is decided as **one core of the target**, and `BLEND_DIFFICULTY_BASE` moves `field_modulus / 2**22` → **`field_modulus / 2**19`**: ~50 s and ~1,750 messages/day per core, 4× on the board, optimiser's edge 1.94×. §4.5's every remaining estimate is thereby replaced by measurement. The genesis reward target keeps `field_modulus / 2**26` (a seed, not a price), now ~3 hours per solution and ~3,700 target-cores at the target rate.
...
- **The body's price-level framing is vindicated.** §4.4.4's correction — that the endowment turns on the *price level*, not the denomination — was right, and is now the operative frame: `claim_fee` at the resting floor is 11,298 lepta ≈ 1.13×10⁻⁵ LGO, about 1.1×10⁻¹⁵ of supply, and the genesis fee ceiling of `1.157×10⁻¹⁰` of supply (unchanged, having been stated supply-relative) binds only if discovered prices rise nearly five orders of magnitude above the floor.
- **Every ratio stands, again**: `fee_ratio` = 0.837, `reward_over_fee` = 5.02 at reference traffic, the 1.124× builder edge, `target_claims_per_block`↔`pow_share`, the 1,000-claims drain margin, the 5.75×10⁻⁵-of-pool genesis-error cost. The whole calibration of §§4.4.1–4.4.3 is untouched. *(These are the values of this note's date; the 2026-09 claim signature later moved the fee-derived ones — 0.494, 2.96, 1.255× — without disturbing the structural point.)*
...
- **Every ratio stands.** `fee_ratio` = 0.837, `reward_over_fee` = 5.02 at the reference traffic, the 1.124× builder edge, the `target_claims_per_block`↔`pow_share` trade, the fee-overhead identity, the stopping conditions, and every argument built on them are supply-free and unchanged. *(Values of this note's date; see the 2026-09 remark above.)*
- **The fee is now determined, not assumed.** The body treats `claim_fee` = 0.952 LGO as a price-level assumption (§5.1). It became `(306 + 646) × 7 = 6,664` lepta at the resting price — a consequence of the fee schedule and the indivisible LGO, with no free parameter left — and the 2026-09 claim signature then moved the same derivation to `(434 + 1,180) × 7 = 11,298` lepta.
...
where `fee_ratio` ≈ 0.494 is the average transaction's fee over a claim's (§4.3). At the specified `target_claims_per_block = 10` that needs about 203 transactions per block at a 10 % share, or 101 at 20 %, comfortably inside what a block can carry at every share worth considering. Before that point the genesis endowment carries the reward.
...
**The claim target is overhead, not throughput.** Each claim pays a fee out of its own reward, so an epoch delivers, net of the fees the claims themselves pay:

`blocks_per_epoch * claim_fee * (fee_ratio * pow_share * txs_per_block - target_claims_per_block)`

`target_claims_per_block` enters with a minus sign. At the earlier `target_claims_per_block = 50` half of everything the pool distributed was returned as fees on the claims themselves. `target_claims_per_block = 10` cuts that to a tenth at the same share, and §4.4.1 works through why the intuition that a higher target onboards more people is backwards.
...
Its normative section §5.8 describes something else — a three-way split of the **block reward**, with:

`block_reward_blend_share + block_reward_leader_share + block_reward_pow_share = 1`

and `distribute_block_reward(block)` taking `reward = get_block_rewards(block)`, illustrated at 59/39/2 of 100.
...
3. **Miners grind.** A miner hashes candidate keys until one lands below the difficulty target — trial and error, which is the point: it costs electricity.
...
Two documents write $T$ for different quantities, which is the reason this document prefers names to letters. Here $T$ is `target_claims_per_block`, the target claims **per block**. In `block-rewards.md` the same letter means $W$, the fee-average look-back window of 120 blocks — a quantity this model does not use at all.
...
| `rate_num` / `rate_den` | $\rho_\text{num}$ / $\rho_\text{den}$ | `EPOCH_POW_DISTRIBUTION_RATE_NUM` / `_DEN` | the integer pair `distribution_rate` is held as, so the protocol never divides (§2.1) |
...
| `next_difficulty_target` / `next_expected_claims` / `next_demand_est` | $d_{n+1}$ / $x_{n+1}$ / $\widehat{\text{dem}}_{n+1}$ | the retarget's outputs for the following block (§3.6) |
...
| `cost_per_candidate` | $\kappa$ | what one candidate costs a miner to try (**unknown**, §1) |
...
**2.1 Price per win.** The epoch's reward is the fraction `distribution_rate` of the pool it may spend, divided by the claims an epoch expects:

| **reward per claim** |
| --- |
| $\sigma_e = \lfloor R\,\rho_\text{num} / (\rho_\text{den}\,T\,N_b)\rfloor$ |
| `reward_per_claim = pool * rate_num // (rate_den * target_claims_per_block * blocks_per_epoch)` |

**2.2 Pool.** The pool pays out what the epoch's claims draw and is topped up at the boundary:
...
**2.4 Difficulty.** The retarget is memoryless — it reconstructs demand from the current target rather than storing an average:

| **the retarget** |
| --- |
| $d_{n+1} = T\,P_\text{ema}\,d_n / \max(1, (P_\text{ema}-F_\text{ema})\,c_n + F_\text{ema}\,T)$ |
| `next_difficulty_target = target_claims_per_block * smoothing_precision * difficulty_target // max(1, (smoothing_precision - smoothing_factor) * claims_in_block + smoothing_factor * target_claims_per_block)` |

capped at `field_modulus - 1`, with `smoothing_precision = 10, smoothing_factor = 9`. This is *not* the controller in proposal §5.7.

**2.5 Arrivals.** Claims land in a block in proportion to hashrate and to how loose the target is:

| **claims per block, in expectation** |
| --- |
| $c_n = H\,\Delta_b\,d_n / p$ |
| `claims_in_block = hashrate * block_seconds * difficulty_target / field_modulus` |
...
**A2 — Wins arrive randomly and independently (Poisson).** `ASSUMED` Spread around the average is its square root, so relative noise is `1 / sqrt(target_claims_per_block)`: **32 % at the specified `target_claims_per_block = 10`**, against 14 % at `target_claims_per_block = 50`. This is the whole quantitative case for a larger `target_claims_per_block`, and §4.4.1 prices what that case costs. **The simulator uses the mean, not samples**, which is why §4.8 runs the arrivals: sampled, the per-block spread is 32.3 % as this assumption says, it is corrected rather than merely averaged away before it reaches the pool, and it never approaches §3.8's drain margin. Sampling also exposes one thing the mean hides — the retarget settles at:

`target_claims_per_block + (smoothing_precision - smoothing_factor) / (2 * smoothing_precision)`

half a percent above target at `target_claims_per_block = 10`. **Risk: low, and now measured.**
...
**A10 — Claims always fit in a block.** `ASSUMED` Blocks hold 1024 and `target_claims_per_block = 10`, so claims are 1.0 % of a full block. **Comfortable with room to spare**, and §4.2 shows it is what keeps self-dealing harmless.
...
**Step 1 — the payout is a fixed fraction.** At the target rate:

| **what an epoch pays out** |
| --- |
| $c_e\,\sigma_e = T N_b \cdot R\rho/(T N_b) = \rho R$ |
| `claims_paid * reward_per_claim == distribution_rate * pool` |

The central identity: **`target_claims_per_block` and `blocks_per_epoch` vanish from the pool dynamics entirely.**

**Step 2 — solve.** The pool obeys an affine map:

| **the pool from one epoch to the next** |
| --- |
| $R_{e+1} = (1-\rho)R_e + F$ |
| `pool = (1 - distribution_rate) * pool + epoch_refill` |

One fixed point, found by setting the two sides equal:

| **the pool's fixed point** |
| --- |
| $R^\ast = F/\rho$ |
...
Two sides have to agree. **Protocol side:** the thermostat holds claims at `target_claims_per_block` whatever hashrate appears, so:

`hashrate * block_seconds * equilibrium_difficulty / field_modulus = target_claims_per_block`

**Miner side:** the expected cost of one win is:

`(field_modulus / difficulty_target) * cost_per_candidate`

for a per-candidate cost `cost_per_candidate`, and free entry drives the marginal miner's margin to zero, so:
...
`equilibrium_hashrate > 0` requires `reward_per_claim > claim_fee`. **The on-ramp survives long-run iff:**

`epoch_refill / (target_claims_per_block * blocks_per_epoch) > claim_fee`

Substituting the refill, `steady_reward > claim_fee` becomes:

`fee_ratio * pow_share * txs_per_block / target_claims_per_block > 1`

§4.3's condition, reached from the miner's side rather than the pool's. The on-ramp and the pool's solvency turn out to be the same inequality.
...
The map carries neither `hashrate` nor `field_modulus`, so its behaviour is a property of the controller alone. Its fixed points solve

| **where the map sits still** |
| --- |
| $x\,(x-T) = 0$ |
| `expected_claims * (expected_claims - target_claims_per_block) == 0` |

and the slopes there settle the question:
...
**The specified retarget is this controller, exactly.** One invariant holds from genesis, because a single initialization sets both sides:

| **the invariant** |
| --- |
| $\widehat{\text{dem}} = T/d$ |
| `demand_est = target_claims_per_block / difficulty_target` |

Substituting it into the EMA gives the smoothed estimate in closed form,

| **the estimate, after substitution** |
| --- |
| $\widehat{\text{dem}}' = [(1-q)c + qT]/d$ |
| `next_demand_est = ((1 - smoothing) * claims_in_block + smoothing * target_claims_per_block) / difficulty_target` |

and inverting it recovers the specification's own map:
...
**Condition 1 — within an epoch: `pool < reward_per_claim`.** `reward_per_claim` is frozen at the epoch boundary while the pool drains with every claim, so after `claims_so_far` claims the pool is:

`genesis_pool - claims_so_far * reward_per_claim`

and the guard fails once that count reaches:
...
**Condition 2 — across epochs: `reward_per_claim = 0`.** The pool decays over many epochs until:

`distribution_rate * pool / (target_claims_per_block * blocks_per_epoch)`
...
where `honest_stake_frac` is the fraction of winnings honest miners stake — **33 % at `adversary_hashrate` = 0.33 with full honest staking, 49.6 % if honest miners stake only half**. There is no horizon at which it turns around.
...
| --- | --- |
| **102,444× — the specified launch value** | **1.0000×** |
| 100× | 1.005× |
| 10× | 1.06× |
| **2.96× — the specified steady state** | **1.255×** |
| 2× | 1.50× |
| 1.5× | 2.00× |
| 1.2× | 3.50× |

**The trajectory depends on where the endowment sits relative to the pool's fixed point**, and at the specified parameters it sits far *above* it (`genesis_pool / steady_pool = 34,576`, §4.12), so the reward decays and **the edge grows**: 1.0000× at launch, 1.037× at twenty years, 1.255× at the steady state (§3.7, §4.7.1).

**The worst moment is therefore the steady state, decades out — not the first epoch.** What makes this benign is not the shape but the bound: the edge is capped by `1 + tip_fraction / (reward_over_fee - 1)` at the settled margin, and 1.255× is small. The shape does mean the concern arrives slowly and cannot be checked against launch behaviour, which is the argument for treating `reward_over_fee` rather than `opening_reward_over_fee` as the number that matters.
...
| **encoded `SignedMantleTx`** | **434 B** | **1,180** |

At the resting price that is `434 * 7 + 1,180 * 7 = 11,298` price units — or 1,614 at the bare floor, which the markets reach only transiently. (Until 2026-09-04 the claim carried no proof and cost 6,664; the ZkSignature and its 590 gas are the whole difference.) An ordinary one-in one-out `TRANSFER` comes to 207 B and 590 gas, or 5,579 units at rest, so

| **the fee ratio** |
| --- |
| $\psi = \dfrac{\bar\varphi}{\varphi_\text{claim}} \approx 0.494$ |
| `fee_ratio = avg_tx_fee / claim_fee  # 0.494` |
...
The pool settles where the refill and the payout balance, and at that point the reward per claim is just the refill divided by the claims an epoch expects:

| **the reward the pool settles at** |
| --- |
| $\sigma^\ast = F/(T\,N_b)$ |
| `steady_reward = epoch_refill / (target_claims_per_block * blocks_per_epoch)` |

Substituting the refill from Background B:
...
which puts the floor where the two meet:
...
| --- | --- | --- | --- | --- |
| 1 | 1,614 | 1.6×10⁻⁷ | **697 %** | impossible |
| 10³ | 1.614 | 1.6×10⁻¹⁰ | **0.70 %** | affordable |
| 10⁶ | 9.5×10⁻⁴ | 9.5×10⁻¹⁴ | 0.0004 % | negligible |
| 10⁹ | 9.5×10⁻⁷ | 9.5×10⁻¹⁷ | ~0 | negligible |

**Nine orders of magnitude in the fee, and therefore in the endowment.** What the table really shows is how sharply the sizing depends on the launch fee level, whatever sets it. Every entry scales linearly with `target_claims_per_block`, so the move to `target_claims_per_block = 10` divided this table by five without changing its shape, and the top row — a fee of 1,614 LGO — remains impossible at any endowment. §4.4.4 restates this as the constraint it actually is: a ceiling of about 1.157 LGO on the launch fee, for the specified endowment to open at twice it.

### Sized against an adoption ramp `SIMULATED`

The single-point view answers "what opens at `opening_multiple` fees". The question the endowment exists to answer is different: **how large must the pool be so that claiming stays worthwhile for the whole time it takes the network to grow into self-funding?** `make rewards` ramps traffic logistically from 20 to 1024 transactions per block over a stated horizon and binary-searches the smallest `genesis_pool` keeping `reward_per_claim >= claim_fee` throughout, at `target_claims_per_block = 10`, `distribution_rate = 1 %` and the settled denomination of 10⁹ lepta per LGO (§0.1).

| `pow_share` | 1-year ramp | 2-year | 5-year | 10-year |
| --- | --- | --- | --- | --- |
| 5 % | 5.32×10⁻⁸ | 5.78×10⁻⁸ | 7.46×10⁻⁸ | 1.16×10⁻⁷ |
| **10 %** | **5.21×10⁻⁸** | **5.55×10⁻⁸** | **6.75×10⁻⁸** | **9.46×10⁻⁸** |
| 20 % | 5.10×10⁻⁸ | 5.32×10⁻⁸ | 6.08×10⁻⁸ | 7.67×10⁻⁸ |
| 33 % | 5.02×10⁻⁸ | 5.15×10⁻⁸ | 5.60×10⁻⁸ | 6.50×10⁻⁸ |
| 50 % | 4.95×10⁻⁸ | 5.02×10⁻⁸ | 5.23×10⁻⁸ | 5.63×10⁻⁸ |
...
**The floor is `pool_floor` = 4.88×10⁻⁸ of supply**, shared by every column — the pool must hold that much for a claim to beat its own fee at all, whatever the traffic. Nothing below it is a viable endowment.
...
**Every share now works.** The previous `target_claims_per_block = 50` produced *never* across the whole 5 % row — no endowment of any size kept claiming alive — and required 1.14–7.88 % of supply elsewhere. At `target_claims_per_block = 10` the entire table fits between 4.95×10⁻⁸ and 1.16×10⁻⁷ of supply, and the choice of share is a preference about robustness to slow adoption rather than a viability constraint.
...
**Where `target_claims_per_block = 50` sat.** At `pow_share` = 20 % on 600-transaction blocks the ceiling with the 2× headroom §4.2 wants is exactly:

`fee_ratio * pow_share * txs_per_block / 2 = 50`

`target_claims_per_block = 50` was not near the ceiling; it *was* the ceiling, which is why §4.3's table found `pow_share` = 20 % to be the minimum viable share. Half of everything the pool distributed was being returned as fees on the claims themselves.

**Holding the share fixed at 20 %, lowering `target_claims_per_block`:**

| `target_claims_per_block` | `reward_over_fee` | eaten by fees | reaches miners | builder edge | noise | `genesis_pool` (5-yr ramp, of supply) |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 29.63 | 3 % | 97 % | 1.02× | 100 % | 4.88×10⁻⁹ |
| 5 | 5.93 | 17 % | 83 % | 1.10× | 45 % | 3.04×10⁻⁸ |
| **10** | **2.96** | **34 %** | **66 %** | **1.25×** | **32 %** | **6.75×10⁻⁸** |
| 25 | 1.19 | 84 % | 16 % | 3.70× | 20 % | 1.93×10⁻⁷ |
| 50 | 0.59 | 100 % | 0 % | n/a — under water | 14 % | 4.50×10⁻⁷ |
| 100 | 0.30 | 100 % | 0 % | n/a | 10 % | never |

The fee column is the point: **claim count is a bad proxy for delivery.** `target_claims_per_block = 50` pays five times as many claims as 10 does and delivers nothing — at the 2026-09 claim fee the claim's own fee eats the entire reward at the reference load — while 10 delivers two thirds of every reward net of fees. (An earlier revision of this table carried an onboarding-per-epoch column, measured against the 100,000-LGO Blend minimum stake, from a pre-denomination generator this report can no longer reproduce; it is withdrawn rather than rescaled, with the same note as §4.4.2's.)
...
**Security does not select a `pow_share`.** §4.1's construction gives an adversary with hashrate share `adversary_hashrate` exactly `adversary_hashrate` of whatever the pool paid out, so its share of total stake converges to:

`adversary_hashrate / (adversary_hashrate + (1 - adversary_hashrate) * honest_stake_frac)`

**whatever `pow_share` is**. `pow_share` sets how fast that limit is approached, not the limit. At `adversary_hashrate = 1/3` with honest miners staking their winnings the limit is 33 % and the threshold is never crossed at any `pow_share`; it is crossed only when honest miners largely do not stake, and there `pow_share` merely changes the date. This is a genuine finding and a slightly disappointing one: the parameter that looked like it should be bounded by security is not.
...
| --- | --- | --- | --- | --- | --- |
| 2 % | 0.59 | n/a — under water | — | — | — |
| 5 % | 1.48 | 2.04× | 33 % | — | — |
| **10 %** | **2.96** | **1.255×** | **66 %** | — | — |
| 20 % | 5.93 | 1.101× | 83 % | — | — |
| 33 % | 9.78 | 1.057× | 90 % | — | — |

(The first four columns are §4.3's closed forms at the 2026-09 claim fee. The last two were generated by a pre-denomination basis this report can no longer reproduce; they are withdrawn rather than rescaled, and the five-year-ramp sizing that is reproducible — as a fraction of supply, per share — is the §4.4 table above.)

Note the top row: **the proposal's illustrative 2 % is now under water** at this traffic — the reward falls 41 % short of the claim's own fee (it sat exactly on break-even before the 2026-09 claim signature). Its 2 % was a share of the *block reward*, a different and much smaller base, so the figure does not transfer.

#### The choice

**`pow_share = 10 %`**, as `POW_SHARE = 10` over `SHARE_DEN = 100`. It clears 2× headroom from 405 transactions per block, reaches 3× at the reference 600, holds the builder edge at 1.255× (the 1.1× aspiration would now need a share near 20 %, above the subordination ceiling, so it is bounded rather than chased), keeps mining at 28 % of the leader share, and needs an endowment of 6.75×10⁻⁸ of supply to cover a five-year ramp (§4.4). A denominator of 100 leaves one-percentage-point granularity for later adjustment.

Going to 20 % would halve the edge's excess over one and lift the delivered share from 66 % to 83 %, at the cost of taking mining to 62 % of the leader share — which is no longer "clearly subordinate", and is the reading of the proposal's §1.5 that decides this.
...
| --- | --- | --- | --- | --- | --- |
| 0.2 % | 3.62×10⁻⁷ | 500 | 1.22×10⁻⁷ | 10.3 yr | 5,000 — unreachable |
| 0.5 % | 1.45×10⁻⁷ | 200 | 4.88×10⁻⁸ | 4.1 yr | 2,000 — unreachable |
| **1 %** | **7.23×10⁻⁸** | **100** | **2.44×10⁻⁸** | **2.1 yr** | **1,000 — just reachable** |
| 2 % | 3.62×10⁻⁸ | 50 | 1.22×10⁻⁸ | 1.0 yr | 500 — reachable at half-full blocks |
| 5 % | 1.45×10⁻⁸ | 20 | 4.88×10⁻⁹ | 0.4 yr | 200 — routinely reachable |
...
| --- | --- |
| `pool_floor` — below this a claim no longer beats its own fee | 4.88×10⁻⁸ |
...
| **the specified `genesis_pool`** | **5.00×10⁻³** — 74,119× the 5-year ramp (§4.12) |
...
depends only on denomination-free quantities; the fee `claim_fee` is a fixed **1,614 base units**. So a chosen `genesis_pool` implies a **floor under the denomination**
...
`claim_fee = 434 * P_STR + 1,180 * b_exec`

 **base units**, where `P_STR` and `b_exec` are the two markets' prices. In LGO that is:

`claim_fee = (434 * P_STR + 1,180 * b_exec) / base_units_per_lgo`
...
is **349 % of the total token supply** at the bare floor and **2,440 %** at the resting price. The pool would have to be many times the entire supply for a claim to beat its own fee. This is not "expensive"; it is impossible at any endowment.
...
1. **Steady-state self-funding, with headroom.**

   `fee_ratio * pow_share * txs_per_block / target_claims_per_block > 2`

   At the specified `target_claims_per_block = 10` and `pow_share = 10 %` this holds from 240 transactions per block up, and the realised headroom at the reference 600 is 5× with a builder edge of 1.124×. The factor of two is §4.2's.
...
so at break-even the claim share is exactly `fee_ratio * pow_share`, and that is the **ceiling**: if claims are more than `fee_ratio * pow_share = 4.94 %` of transactions, a claim earns less than the fee it pays. It depends on `pow_share` and nothing else — `target_claims_per_block` cancels, and so does the traffic level. At the specified set the network operates at `claim_share = 1.67 %`, a **2.96× margin** below the ceiling, and the break-even traffic is 203 tx/block.
...
**Raising `pow_share` does not put more claims in a block.** The difficulty controller holds the claim count at `target_claims_per_block` whatever `pow_share` is, so the claim share of transaction volume is invariant in `pow_share` — the identity above moves the *ceiling*, not the operating point. What `pow_share` buys is traffic headroom: the floor below which mining stops funding itself is:

`target_claims_per_block / (fee_ratio * pow_share)`

which the specified tenth puts at 119 tx/block.

The sweep in §4.4.2 is worth re-reading against this. **At the proposal's original 2 % example the break-even traffic is 1,013 tx/block against a reference of 600** — `reward_over_fee = 0.59`, under water at the reference load; before the 2026-09 claim fee it sat at 597 against 600, marginal rather than sunk. The move to a tenth is what converts the mechanism from under-water to funded.
...
Each ramp is plotted at *its own* minimum endowment, where each just grazes the floor — which is what makes the test legible. At the specified `genesis_pool` all four curves lie on top of one another, because the endowment is **74,119× the 5-year minimum** and the ramp shape disappears beneath it.
...
> **A block must collect `target_claims_per_block / pow_share` claim fees for mining to fund itself — 100 at the specified set.**
...
| --- | --- | --- | --- | --- |
| 25 | 0.25 | under water | 282,450 | 51 |
| 50 | 0.50 | under water | 564,900 | 101 |
| **100** | **1.00** | **break-even** `target_claims_per_block / pow_share` | 1,129,800 | 203 |
| 200 | 2.00 | works, 2× margin | 2,259,600 | 405 |
| **296** | **2.96** | **specified** | 3,344,208 | 599 |
| 506 | 5.06 | full block | 5,716,788 | 1,024 |
| 2,000 | 20.00 | ample | 22,596,000 | 4,050 |

The specified set collects **296 claim fees per block against a break-even of 100** — the same 2.96× margin §4.3 derives, reached without an estimate of either traffic or price. Read as a count that is 600 ordinary transfers at the resting level; read as a price level it is anything at all. **That invariance is the substance, not a presentational convenience**: it is why §4.3's ratios survive a repricing, and why the "the on-ramp closes when the network is busiest" failure mode does not exist under fee funding. `make verify` checks it across five decades of price.
...
| --- | --- | --- | --- |
| 600 transfers | 296 | 2.96 | the reference |
| 10 claims + 590 transfers | 504 | 5.04 | the realistic block — and the 0.32 % §4.7.2 flags, made concrete |
| a full block of transfers | 506 | 5.06 | |
| a full block of the cheapest Operation | 200 | 2.00 | |
| half a block, half inscribes | 264 | 2.64 | |

**The composition barely matters; the fill does.** A full block needs only `(target_claims_per_block / pow_share) / MAX_BLOCK_TXS` = **0.098 claim fees per transaction** to break even, which at the cheapest Operation's 56 gas is about **102 encoded bytes each** — smaller than any signed transaction can be, since a signature alone is 128 B. So every full block clears break-even whatever is in it, though the margin has thinned: a block filled entirely with the cheapest Operation now lands at 1.18× the floor, where it cleared twice the floor before the 2026-09 claim fee. What moves the margin is how *full* blocks are, and the specified 2.96× corresponds to blocks a little under 60 % full of transfers.
...
- **Reconvergence is 22 blocks in every cell.** §3.6 predicted "~22" for a tenfold hashrate step from the pole `smoothing_factor / smoothing_precision`; the simulation gives exactly 22, and it does not move with `target_claims_per_block`, `pow_share`, `distribution_rate` or `genesis_pool`, because the controller's normalised dynamics contain none of them. (Recovery is asymmetric, as §4.7.5 shows: a tenfold step *down* takes 42 blocks.)
- **The security column does not move with `target_claims_per_block`.** An epoch distributes `distribution_rate * pool` whatever `target_claims_per_block` is — `target_claims_per_block` cancels out of:

   `target_claims_per_block * blocks_per_epoch * reward_per_claim`

   so the attacker's share is a property of the pool and the horizon, not of the claim target. This is §3.1's identity showing up where it should.
- **`reward_over_fee` does not move with `distribution_rate` or `genesis_pool`.** The steady state is:

   `epoch_refill / (target_claims_per_block * blocks_per_epoch)`

   which contains neither. §3.1's "`distribution_rate` sets the speed, never the destination" holds numerically.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **10** (specified) | **2.96** | 74,119× | 0.42 % | **1.255×** | 2,000 | 100 | 1.7 % |
| 50 | 0.59 | 11,111× | 0.42 % | n/a — under water | 10,000 | 500 | 8.3 % |
| 100 | 0.30 | never | 0.42 % | n/a | 20,000 | 1,000 | 16.7 % |
| 500 | 0.06 | never | 0.42 % | n/a | 100,000 | 5,000 | 83.3 % |
...
**`target_claims_per_block = 50` is under water at the reference load since the 2026-09 claim fee** — `reward_over_fee = 0.59`, the reward falls 41 % short of the claim's own fee. Before that change it sat almost exactly on break-even, which is where the builder edge explodes: the edge is `1 + tip_fraction / (reward_over_fee - 1)`, a pole at break-even (it read 109× there), and the pole now sits at the break-even target of about 30 — `target_claims_per_block = 29` still clears at 1.02, 30 dips under. Either way `target_claims_per_block = 50` is disqualified outright, which is a sharper argument for the move to 10 than §4.2's, which put the earlier value's edge at 2.5× on a traffic assumption the current set does not carry.

Above the break-even target of about 30 the mechanism is simply under water: `reward_over_fee < 1` at the reference load (0.59 already at 50), the ramp cover dies past 100, and at 500 claims would need to be **83 % of every block**. When this sweep was first run at `distribution_rate = 1/100`, raising `target_claims_per_block` bought the one thing the specified set then lacked — `target_claims_per_block / distribution_rate` passed `MAX_BLOCK_TXS` between 10 and 50, closing the within-epoch drain structurally — at the cost of everything else. **§0.4 took the other route to the same guarantee**: `distribution_rate` = 1/200 closes the drain at `target_claims_per_block = 10`, so a larger `target_claims_per_block` now buys nothing at all, and the specified 10 keeps its margin everywhere.

The two `1 / target_claims_per_block` effects run the other way and are small: arrival noise falls from 31.6 % to 4.5 % (§4.8's A2 case), and the retarget's overshoot from 0.50 % to 0.01 %.
...
![the target_claims_per_block-pow_share plane](figures/11_T_beta_plane.png)
...
| --- | --- | --- |
| `target_claims_per_block` alone | within-epoch drain, `target_claims_per_block / distribution_rate` vs `MAX_BLOCK_TXS` | 1,000 vs 1,024 — **reachable** |
...
**Unknown.** The launch fee level, i.e. what genesis governance initialises the two market prices to — this is what §4.4's table was really varying, and §4.4.4 restates it as a ceiling governance must hit rather than an open unknown. `cost_per_candidate`. The adoption rate, which §4.4 handles by parameterising rather than guessing.
...
| --- | --- | --- | --- | --- |
| `field_modulus / 2**20` | 1,048,576 | 24.5 s | 3,529 | 35 M |
| **`field_modulus / 2**22`** | **4,194,304** | **97.9 s** | **882** | **8.8 M** |
| `field_modulus / 2**24` | 16,777,216 | 6.5 min | 221 | 2.2 M |
| `field_modulus / 2**26` | 67,108,864 | 26.1 min | 55 | 0.55 M |
...
| --- | --- | --- | --- | --- | --- | --- |
| `field_modulus / 2**19` | 12 s | 49 s | 73 s | 98 s | 1,176 | 4,706 |
| `field_modulus / 2**20` | 24 s | 98 s | 2.4 min | 3.3 min | 588 | 2,353 |
| **`field_modulus / 2**22`** (specified) | **98 s** | **6.5 min** | **9.8 min** | **13.1 min** | **147** | **588** |
...
**Smoothing: `smoothing_factor = 9`, `smoothing_precision = 10`.** The same ratio the execution market uses. §3.6 showed the response has slope `smoothing_factor / smoothing_precision` at the target — below one, hence stable, with a time constant of about ten blocks — and slope `smoothing_precision / smoothing_factor` at zero claims, above one, so the no-claims state repels rather than traps. **Both signs hold for any `smoothing_factor < smoothing_precision`**, so the pair chooses response *speed*, not stability. Nine in ten places the response an order of magnitude slower than the block rate: fast enough to track a hashrate change within minutes, slow enough that one unusual block barely moves the target. It also matches what the merged code ships, so this is transcription with a reason attached rather than a fresh choice.

**Genesis target: `field_modulus / 2**26`.** What matters here is not the value but the **asymmetry of being wrong**, simulated against the actual controller:
