...
- **The Blend candidate is three Poseidon2 permutations, and it was already measured.** The Pi 5 runs of 2026-08-12 timed a two-input `zkhash` at **71.3 μs** (median of six, spread 0.05 %), and input order does not change the cost, so `[work]` now carries that line rather than the 94.2 μs of the four-permutation v0.5.6 form. At the specified `field_modulus / 2**19` a solution is ~37 s and ~2,300 messages/day per core, 4× on the board. Whether the exponent moves to restore the ~50 s the specification's calibration sentence still states is a decision for the RFC, not this model.
- **The reward candidate is unchanged in cost**: the key derivation plus a three-input ticket, seven permutations, 165.7 μs measured; only the constant `KDF` tag can be absorbed ahead of time (one permutation, ~14 %, derived). The genesis reward target keeps `field_modulus / 2**26`, ~3.1 hours per solution on one core.
