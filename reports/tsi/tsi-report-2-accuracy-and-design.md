...
![Fig 1 — bootstrap at full scale (k=2160, Blend, N=1000): from any genesis guess (0.01×–2×), with uncles (solid, U=2) block production (top) and the estimate (bottom) converge to the target f and D̂/D = 1.0 within ~2 epochs; without uncles (dashed, U=0) they settle more slowly to a chronically overheated, guess-dependent equilibrium — D̂/D ≈ 0.48–0.85 (mean ≈ 0.58; ≈ 0.51 at N=5000), block rate ≈ 1.2–2.1×f.](report-figures/fig1_bootstrap.png)
...
![Fig 16 — recovered relative stake D̂/D vs mean visibility delay D_vis (Blend, f=1/30): each uncle cap U holds accuracy at 1.0 until ρ=f·D_vis exceeds U, marked at ρ=1, 2.](report-figures/fig16_stake_vs_delay.png)
...
![Fig 19 — direct-gossip (regular, N=10000) accuracy vs mean per-link latency at U=0: at the single tested 0.5-slot per-link latency, U=0 alone recovers 0.97 (degree 6) / 0.93 (degree 4); higher peering degree helps.](report-figures/fig19_accuracy_vs_latency.png)
...
![Fig 26 — the region below the block rate: under-count deficit `1 − D̂/D` vs load ρ per uncle cap (left, log-y — the `U = ⌈ρ⌉` boundary), and accuracy capped at the `D̂/D = 1` bound (right — the equilibrium sits at or below 1 at every load, U=2/U=3 hugging 1 from below).](report-figures/fig26_deficit_vs_rho.png)
...
![Fig 25 — window sufficiency at scale (N = 1 000 solid-circle vs 10 000, U = 1 solid / U = 2 dashed): the floor position is N-invariant (left), W = 600 buys back the ρ ≈ 1 boundary (middle), and no window fixes ρ > U (right).](report-figures/fig25_window_scale.png)
...
![Fig 20 — accuracy decision chart D̂/D over (blending delay × uncle cap) at N=10000, degree 6: U=0 under-counts (0.67–0.75), U≥1 recovers to exactly 1.00 across the tested delay range.](report-figures/fig20_heatmap_accuracy.png)
