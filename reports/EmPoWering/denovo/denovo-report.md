...
This requirement is the reason the design exists, so it gets the sharpest test: a cohort of **13,000 nodes — a hundred times the background arrival rate — lands in one epoch**. The epoch saturates at block 253 of 21,600, and the borrow-forward pays **on the order of a hundred times the epoch's budget** without ceremony — 86× to 111× across draws, the spread being Pareto-tail luck among 13,000 hashrates — roughly half the remaining endowment, brought forward for a cohort that then repays the schedule by finishing it one epoch early, 196 instead of 195. (The ×10 cohort, for scale, needs 2.6× its budget.) The index reprices the next epoch.
