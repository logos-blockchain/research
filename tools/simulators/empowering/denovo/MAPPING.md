...
| --- | --- | --- |
| claim operation `CLAIM_POW_REWARD` | `mantle` §Proof of Work Operations | payload, ticket derivation, nullifier, acceptance window, and since 2026-09-04 a ZkSignature proof at 590 gas and a one-epoch nonce straddle — byte-for-byte. The window (`W = 10` expected blocks) is also **priced**, not just consumed: free only while the field retires; against MODEL §8.3's claim room it taxes the ×100 spike hard (76.9% expiry, ×4.33 energy) and dominates the late persistent endgame (87.8% expiry, ×8.21 at epoch 194), moves every retirement threshold above a cent — earlier exits, more onboarding — costs 1.05%/epoch in the post-phase tail, and bounds any attacker's stockpile to ~10 blocks of its own rate (`window.py`, gated; report §4) |
